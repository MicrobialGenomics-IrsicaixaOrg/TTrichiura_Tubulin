...
========================================================================================
 MicrobialGenomics/TTrichiura_Tubulin Analysis Pipeline.
 #### Homepage / Documentation
 https://github.com/MicrobialGenomics/TTrichiura_Tubulin
----------------------------------------------------------------------------------------
*/
...
def helpMessage() {
    ...
    nextflow run MicrobialGenomics/TTrichiura_Tubulin --input samplesheet.csv --genome 'MN908947.3' -profile docker

    Mandatory arguments
      --input [file]                    Comma-separated file containing information about the samples in the experiment (see docs/usage.md)
      --fasta [file]                    Path to fasta reference for Trichiura genome. Mandatory when --genome not supplied
    ...
    References                          If not specified in the configuration file or you wish to overwrite any of the references
      --genome [str]                    Name of genome reference key for Trichiura genome (Default: '')
      --gff [file]                      Full path to Trichiura gff annotation file (Default: '')
...
if (params.fasta) {
    ...
    exit 1, "Trichiuragenome fasta file not specified!"
}
...
summary['Trichiura Genome']              = params.genome ?: 'Not supplied'
summary['Trichiura Fasta File']          = params.fasta
if (params.gff)                      summary['Trichiura GFF'] = params.gff
...
// Print warning if Trichiura genome fasta has more than one sequence
def count = 0
ch_fasta.withReader { reader ->
    while (line = reader.readLine()) {
        if (line.contains('>')) {
            count++
            if (count > 1) {
                log.info "[MicrobialGenomics/TTrichiura_Tubulin] WARNING: This pipeline does not support multi-fasta genome files. Please amend the '--fasta' parameter."
                break
            }
        }
    }
...
process CHECK_SAMPLESHEET {
    ...
    script:  // These scripts are bundled with the pipeline, in MicrobialGenomics/TTrichiura_Tubulin/bin/
...
/*
 * STEP 4: adapter trimming and quality filtering
 */
if (!params.skip_adapter_trimming) {
    process TRIMMOMATIC {
        tag "$sample"
        ...
        publishDir "${params.outdir}/preprocess/trimmomatic", mode: params.publish_dir_mode,
            saveAs: { filename ->
                          if (filename.endsWith("_fastqc.html")) "fastqc/$filename"
...
/*
 * PREPROCESSING: Build SnpEff database for Trichiura genome
 */
...
process BOWTIE2 {
    ...
    input:
    tuple val(sample), val(single_end), path(reads) from ch_trimmomatic_bowtie2
...
Channel.from(summary.collect{ [it.key, it.value] })
    ...
    .map { x -> """
    id: 'TTrichiura_Tubulins-ummary'
    description: " - this information is collected when the pipeline is started."
    section_name: 'MicrobialGenomics/TTrichiura_Tubulin Workflow Summary'
    section_href: 'https://github.com/MicrobialGenomics/TTrichiura_Tubulin'
    plot_type: 'html'
    data: |
...
process get_software_versions {
    ...
    trimmomatic PE -version > v_trimmomatic.txt
...
workflow.onComplete {
    ...
    def subject = "[MicrobialGenomics/TTrichiura_Tubulin] Successful: $workflow.runName"
    if (fail_mapped_reads.size() > 0) {
        subject = "[MicrobialGenomics/TTrichiura_Tubulin] Partially Successful (${fail_mapped_reads.size()} skipped): $workflow.runName"
    }
    if (!workflow.success) {
        subject = "[MicrobialGenomics/TTrichiura_Tubulin] FAILED: $workflow.runName"
    }
    def email_fields = [:]
    ...
    try {
        if (workflow.success) {
            ...
            if (mqc_report.getClass() == ArrayList) {
                log.warn "[MicrobialGenomics/TTrichiura_Tubulin] Found multiple reports from process 'multiqc', will use only one"
                mqc_report = mqc_report[0]
            }
        }
    } catch (all) {
        log.warn "[MicrobialGenomics/TTrichiura_Tubulin] Could not attach MultiQC report to summary email"
    }
    ...
    if (email_address) {
        try {
            ...
            log.info "[MicrobialGenomics/TTrichiura_Tubulin] Sent summary e-mail to $email_address (sendmail)"
        } catch (all) {
            // Catch failures and try with plaintext
            [ 'mail', '-s', subject, email_address ].execute() << email_txt
            log.info "[MicrobialGenomics/TTrichiura_Tubulin] Sent summary e-mail to $email_address (mail)"
        }
    }
    ...
    if (pass_mapped_reads.size() > 0) {
        ...
        //log.info "[${c_purple}MicrobialGenomics/TTrichiura_Tubulin${c_reset}] ${c_green}${pass_mapped_reads.size()}/${total_count} samples passed minimum mapped reads check\n${sample_mapped}${c_reset}"
    }
    if (fail_mapped_reads.size() > 0) {
        sample_mapped = ''
        ...
        log.info "[${c_purple}MicrobialGenomics/TTrichiura_Tubulin${c_reset}] ${c_red} WARNING - ${fail_mapped_reads.size()} samples skipped due to low number of mapped reads!\n${sample_mapped}${c_reset}"
    }
    ...
    if (workflow.success) {
        log.info "-${c_purple}[MicrobialGenomics/TTrichiura_Tubulin]${c_green} Pipeline completed successfully${c_reset}-"
    } else {
        checkHostname()
        log.info "-${c_purple}[MicrobialGenomics/TTrichiura_Tubulin]${c_red} Pipeline completed with errors${c_reset}-"
    }
...
def nfcoreHeader() {
    ...
    ${c_blue}  | \\| |       \\__, \\__/ |  \\ |___     ${c_green}\\`-._,-`-,${c_reset}
                                            ${c_green}`._,._,\'${c_reset}
    ${c_purple}  MicrobialGenomics/TTrichiura_Tubulin v${workflow.manifest.version}${c_reset}
    -${c_dim}--------------------------------------------------${c_reset}-
    """.stripIndent()
}
