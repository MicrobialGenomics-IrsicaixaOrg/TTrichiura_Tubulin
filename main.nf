...
========================================================================================
                         nf-core/tt
========================================================================================
 nf-core/tt Analysis Pipeline.
 #### Homepage / Documentation
 https://github.com/nf-core/tt
----------------------------------------------------------------------------------------
*/
 ...
 */
if (!params.skip_adapter_trimming && params.fastp) {
    process FASTP {
        tag "$sample"
        label 'process_medium'
        ...
        input:
        tuple val(sample), val(single_end), path(reads) from ch_cat_fastp_trimmomatic

        output:
        tuple val(sample), val(single_end), path("*.trim.fastq.gz") into ch_fastp_trimmomatic_bowtie2
        path "*.json" into ch_fastp_trimmomatic_mqc
        path "*_fastqc.{zip,html}" into ch_fastp_trimmomatic_fastqc_mqc
        path "*.{log,fastp.html}"
        path "*.fail.fastq.gz"
...
} else if (!params.skip_adapter_trimming && !params.fastp) {
    process TRIMMOMATIC {
        tag "$sample"
        label "process_medium"
        ...
        output:
        tuple val(sample), val(single_end), path("*.trim.fastq.gz") into ch_fastp_trimmomatic_bowtie2
        path "*.log" into ch_fastp_trimmomatic_mqc
        path "*_fastqc.{zip,html}" into ch_fastp_trimmomatic_fastqc_mqc
        ...
        if $single_end; then 
            ...
            trimmomatic SE -threads $task.cpus -phred33 \${IN_READS} \${OUT_READS} \\
                ILLUMINACLIP:/opt/conda/envs/nf-core-viralrecon-1.1.0/share/trimmomatic-0.39-1/adapters/TruSeq3-SE.fa:2:30:10 \\
            ...
            trimmomatic PE -threads $task.cpus -phred33 \${IN_READS} \${OUT_READS} \\
                ILLUMINACLIP:/opt/conda/envs/nf-core-viralrecon-1.1.0/share/trimmomatic-0.39-1/adapters/TruSeq3-PE.fa:2:30:10 \\
                LEADING:30 TRAILING:30 MINLEN:75 SLIDINGWINDOW:30:20 2> ${sample}.trimmomatic.log
    ...
    ch_cat_fastp_trimmomatic
        .into { ch_fastp_trimmomatic_bowtie2 }
    ch_fastp_trimmomatic_mqc = Channel.empty()
    ch_fastp_trimmomatic_fastqc_mqc = Channel.empty()
}
...
process BOWTIE2 {
    ...
    input:
    tuple val(sample), val(single_end), path(reads) from ch_fastp_trimmomatic_bowtie2
